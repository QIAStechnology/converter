...
def load_csv(file_path, limit=None):
    ...
    with open(file_path, newline="", encoding="latin-1") as csvfile:
        reader = csv.DictReader(csvfile, delimiter=";")  # keep headers as keys

        for i, row in enumerate(reader):
            # Keep the row as-is (all header columns preserved)
            product = {k.strip(): (v.strip() if v is not None else "") for k, v in row.items()}
...
def main():
    ...
    print("printing first 5 products from CSV:", csv_products[:5])
